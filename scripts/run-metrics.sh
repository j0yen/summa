...
if [ "$AC_TOTAL" -gt 0 ]; then
  # Each tests/acceptance_ac<N>.rs is its own cargo test binary; cargo's
  # `--test` output frames its results between a "Running tests/<file>.rs"
  # header and the next such header. The #[test] fns inside are named
  # ac<N>_<description> (NOT prefixed with "acceptance_" — only the file is),
  # so a naive `grep '^test acceptance_'` always matches zero regardless of
  # true pass state (reviewer-agent finding, 2026-09-06: metrics.json reported
  # ac_passing_count=0/10 with all ten acceptance tests green). Instead, frame
  # by the "Running tests/acceptance_ac*.rs" header and count a frame as
  # passing only if none of its test lines are FAILED — this stays scoped to
  # this PRD's own acceptance_ac*.rs binaries even though other test binaries
  # in this repo (e.g. tests/relevant_ac*.rs) also happen to name fns ac<N>_*.
  AC_PASSING=$(awk '
    /^     Running (target\/[a-z]+\/deps\/)?tests\/acceptance_ac[0-9]+\.rs/ {
      if (in_frame && !failed) passing++
      in_frame=1; failed=0; next
    }
    /^     Running / {
      if (in_frame && !failed) passing++
      in_frame=0; next
    }
    in_frame && /^test .* \.\.\. FAILED$/ { failed=1 }
    END { if (in_frame && !failed) passing++; print passing+0 }
